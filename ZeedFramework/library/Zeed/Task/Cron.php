...
class Zeed_Task_Cron
{
     ...
     * @return integer|boolean|mixed $cronString
     */
    public static function parseCron($cronString = null)
    {
        $cronParser = new self();
        if($cronParser->calcLastRan($cronString)){
            return $cronParser->getLastRanUnix();
        }
    ...
    private function _getMinutesArray()
    {
        if (empty($this->minutes_arr)) {
            ...
            } elseif (preg_match('/^\*\/([0-9]{1,})/', $this->bits[0], $matchs)) {
                for($i = 0; $i <= 59; $i = $i + $matchs[1]) {
                    $minutes[] = $i;
                }
            } else {
