 ...
 * @version SVN: $Id$
 */
class AdminAbstract extends Zeed_Controller_Action
{
    ...
    {
        $this->_resultSmartyType[$result][$resultType] = array(
                'type' => $resultType,
                'resource' => $resource
        );
    ...
    public function _commonPrompt ($arr = array())
    {
        ...
        header("Location: /admin/commonPrompt?{$p}");
        exit();
    }

    protected function _getOffsetArray ()
    {
        $data = array();
        
        $page = (int) $this->input->get('pageIndex', 0);
        
        $data['ordername'] = $this->input->get('ordername', null);
        
        $data['orderby'] = $this->input->get('orderby', null);
        
        $data['perpage'] = (int) $this->input->get('pageSize', $this->_perpage);
        
        $data['offset'] = $page * $data['perpage'];
