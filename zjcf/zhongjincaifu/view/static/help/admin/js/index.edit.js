...
function form_submit() {
	...
	$('textarea[name="help_content"]').val(ckeditor1.getData());
	notice('edit_notice', img_loading_small, false);
	notice('edit_notice', img_loading_small, false);
	
		if (! $("input[name=help_title]").val()) {
			notice('edit_notice', img_delete + ' 帮助名称不能为空', true, 5000);
			return false;
		}
	...
	if (help_id == '' || help_id == 0) {
		...
		$("#edit_form").attr("action", "/helpadmin/index/edit");
