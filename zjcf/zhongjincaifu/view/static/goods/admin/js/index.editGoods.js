...
function form_submit() {
	...
	if (! $("#goods_pattern").val()) {
		notice('edit_notice', img_delete + ' 请选择产品模式', true, 5000);
		return false;
	}
	if (! $("#goods_status").val()) {
	...
	if (! $("input[name=end_time]").val()) {
		notice('edit_notice', img_delete + ' 产品结息日期不能为空', true, 50000);
		return false;
	}
	if (! $("input[name=deal_date]").val()) {
		notice('edit_notice', img_delete + ' 产品兑付日期不能为空', true, 50000);
		return false;
	}
...
//	CKupdate();
	...
	if (goods_id == '' || goods_id == 0) {
		...
		$("#edit_form").attr("action", "/goodsadmin/index/editGoods");
