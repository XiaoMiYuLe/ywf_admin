...
class Util_Image
{
    ...
    function resize()
    {
        if (in_array($this->image_library, array('gd', 'gd2'))) {
    ...
    function setResizeCropProperties($img_w, $img_h)
    {
        ...
        if ($rate_w >= $rate_h) {
            ...
            $this->x_axis_RC = 0;
            $this->y_axis_RC = round(($this->orig_height - $this->orig_height_RC) / 8);
            $this->y_axis_RC = 0;
        } else {
            // 按高比缩放
            $this->orig_width_RC = $this->width_RC * $rate; // 设置截取原图的宽
            $this->orig_height_RC = $this->orig_height; // 设置截取原图的高
            $this->x_axis_RC = round(($this->orig_width - $this->orig_width_RC) / 2);
            $this->y_axis_RC = 0;
        }
    }
    ...
    function image_process_gd($action = 'resize')
    {
        ...
        //  Create The Image
        if ($this->image_library == 'gd2' and function_exists('imagecreatetruecolor') and substr($this->gd_version(), 0, 1) == '2') {
        ...
        //  Show the image
        if ($this->dynamic_output == TRUE && $this->full_dst_path) {
            $this->image_display_save_gd($dst_img);
        } elseif ($this->dynamic_output == TRUE) {
    ...
    function image_process_imagemagick($action = 'resize')
    {
        ...
        // Execute the command
        $cmd = $this->library_path . " -comment \"BY TrendCMS 2008. Author: xSharp@gmail.com\" -quality " . $this->quality;
        ...
        if ($action == 'crop') {
            ...
            $cmd_pre = $cmd . " -crop " . $this->orig_width_RC . "x" . $this->orig_height_RC . "+" . $this->x_axis . "+" . $this->y_axis . " \"$this->full_src_path\" \"$this->full_dst_path\" 2>&1";
            ...
            $this->x_axis = 0;
            $this->y_axis = 0;
            $cmd .= " -resize " . $this->width . "x" . $this->height . "+" . $this->x_axis . "+" . $this->y_axis . " \"$this->full_dst_path\" \"$this->full_dst_path\" 2>&1";
        ...
        } else // Resize
{
    ...
    function image_process_netpbm($action = 'resize')
    {
        ...
        if ($action == 'crop') {
            ...
            $cmd_inner = 'pnmcut -left ' . $this->x_axis . ' -top ' . $this->y_axis . ' -width ' . $this->orig_width_RC . ' -height ' . $this->orig_height_RC;
            $cmd = $this->library_path . $cmd_in . ' ' . $this->full_src_path . ' | ' . $this->library_path . $cmd_inner . ' | ' . $this->library_path . $cmd_out . ' > ' . $this->dest_folder . $tmpfilename . '_pre';
        ...
        } else // Resize
{
        ...
        $cmd = $this->library_path . $cmd_in . ' ' . $this->full_src_path . ' | ' . $this->library_path . $cmd_inner . ' | ' . $this->library_path . $cmd_out . ' > ' . $this->dest_folder . $tmpfilename;
