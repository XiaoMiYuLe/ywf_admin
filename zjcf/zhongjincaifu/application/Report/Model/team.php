...
class Report_Model_team extends Zeed_Db_Model
{
    /**
     * @var string The table name.
     */
    protected $_name = 'tuser_day';
    /**
    ...
    public function fetchByteam($where = null, $order = null, $group=null,$perpage = null, $offset = null, $cols = '*')
    {
    	...
    	$select->join('report_organ_list',"report_organ_list.organ_code = report_tuser_day.organ_code",'organ_name');
    	$select->join('report_team_list',"report_team_list.team_code = report_tuser_day.team_code",array('team_name'));
    	if ($order !== null) {
    		$select->order($order);
    	}
    	if ($group !== null) {
