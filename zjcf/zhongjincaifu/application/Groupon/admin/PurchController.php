...
class PurchController extends GrouponAdminAbstract
{
    ...
    public function index()
    {
        ...
        if ($this->input->isAJAX()) {
            ...
            $bulks = Groupon_Model_Grab::instance()->fetchByWhere($where, $order, $perpage, $offset);
            $data['count'] = Groupon_Model_Grab::instance()->getCount($where);
        ...
        $this->addResult(self::RS_SUCCESS, 'php', 'purch.index');
    ...
    public function add()
    {
        ...
        $this->setData('data', $data);
        $this->addResult(self::RS_SUCCESS, 'php', 'purch.edit');
        return parent::multipleResult(self::RS_SUCCESS);
    }
    ...
    public function addSave()
    {
        ...
        if ($set['status'] == 0) {
            try {
                $bulk_id = Groupon_Model_Grab::instance()->addForEntity($set['data']);
                if (! $bulk_id) {
                    Goods_Model_Content::instance()->update(array('promotion_type'=>3), "sku = '{$set['data']['sku']}'");
    ...
    public function edit()
    {
        ...
        $bulk = Groupon_Model_Grab::instance()->fetchByPK($bulk_id);
    ...
    public function editSave()
    {
        ...
        if ($set['status'] == 0) {
            try {
                Groupon_Model_Grab::instance()->updateForEntity($set['data'], $set['data']['bulk_id']);
                Goods_Model_Content::instance()->update(array('promotion_type'=>3), "sku = '{$set['data']['sku']}'");
            } catch (Zeed_Exception $e) {
                $this->setStatus(1);
    ...
    private function _validate()
    {
        ...
        if(! empty($goods) ) {
            ...
            if($info['price'] < $res['data']['price']) {
                $res['status'] = 1;
                $res['error'] = '抢购价格不能大于商品价格';
                return $res;
            }
            
            if($info['stock'] < $res['data']['inventory'] || $res['data']['buynumber'] > $res['data']['inventory']) {
                $res['status'] = 1;
    ...
    public function delete()
    {
        ...
        try {
            $info = Groupon_Model_Grab::instance()->fetchByPK($id);
            
            Groupon_Model_Grab::instance()->deleteByPK($id);
