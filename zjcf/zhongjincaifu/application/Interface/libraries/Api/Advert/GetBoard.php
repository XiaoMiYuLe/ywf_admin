...
/**
 * 获取广告位 -- 用户前端查看
 * @todo 读取表 advert_board
 */
class Api_Advert_GetBoard
{
    ...
    public static function run($params = null)
    {
        ...
        if ($res['status'] == 0) {
            try {
                ...
                $data      = $redis_val == 1 ? Trend_Model_Redis::instance()->get($cache_key) : 0;
                if (! $data) {
                    // 只是提供商端查看，无条件和排序
                    $where = null;
                    $order = null;
                    $data = Advert_Model_Board::instance()->fetchByWhere($where, $order);
                ...
                $res['status'] = 1;
                $res['error'] = '查询广告位出错。错误信息：' . $e->getMessage();
                return $res;
            }
